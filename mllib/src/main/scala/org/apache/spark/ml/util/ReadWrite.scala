...
abstract class MLWriter extends BaseReadWrite with Logging {
  ...
  def save(path: String): Unit = {
    new FileSystemOverwrite().handleOverwrite(path, shouldOverwrite, sparkSession.sessionState)
    saveImpl(path)
  }
...
private[ml] class FileSystemOverwrite extends Logging {
  ...
  def handleOverwrite(path: String, shouldOverwrite: Boolean, sessionState: SessionState): Unit = {
