...
abstract class BucketedReadSuite extends QueryTest with SQLTestUtils {
  ...
  private def testBucketing(
    ...
    withTable("bucketed_table1", "bucketed_table2") {
      ...
      // Verify that we can collect the datasets when codegen is enabled, in order
      // to test codegen and vectorized readers.
      withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "0",
        ...
        t1.collect()
        t2.collect()
        t1.join(t2, joinCondition(t1, t2), joinType).collect()
      }
