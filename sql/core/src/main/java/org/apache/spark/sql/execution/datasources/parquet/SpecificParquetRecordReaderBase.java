...
public abstract class SpecificParquetRecordReaderBase<T> extends RecordReader<Void, T> {
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    if (rowGroupOffsets == null) {
      ...
      ParquetFileReader reader = ParquetFileReader.open(taskAttemptContext.getConfiguration(),
        file, footer);
      blocks = filterRowGroups(
          ImmutableList
            .of(RowGroupFilter.FilterLevel.STATISTICS, RowGroupFilter.FilterLevel.DICTIONARY),
          filter,
          footer.getBlocks(),
          reader);
    } else {
