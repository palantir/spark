...
import org.apache.spark.sql.execution.datasources.{DefaultCatalogFileIndex, HadoopFsRelation, LogicalRelation}
...
class CachedTableSuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("cache a table using CatalogFileIndex") {
    withTable("test") {
      ...
      val catalogFileIndex = new DefaultCatalogFileIndex(spark, tableMeta, 0)
      ...
      val sameCatalog = new DefaultCatalogFileIndex(spark, tableMeta, 0)
