...
class MesosCoarseGrainedSchedulerBackendSuite extends SparkFunSuite
  ...
  private var executorProvider: MesosExecutorProvider = _
  ...
  test("mesos supports killing and limiting executors") {
    ...
    val minMem = executorProvider.executorMemory(sc)
    val minCpu = 4
    ...
    assert(executorProvider.doKillExecutors(Seq("0")).futureValue)
  ...
  test("mesos supports killing and relaunching tasks with executors") {
    ...
    // launches a task on a valid offer
    val minMem = executorProvider.executorMemory(sc) + 1024
    val minCpu = 4
    ...
    executorProvider.statusUpdate(driver, status)
  ...
  test("mesos supports spark.executor.cores") {
    ...
    val executorMemory = executorProvider.executorMemory(sc)
    ...
    val cpus = executorProvider.getResource(taskInfos.head.getResourcesList, "cpus")
  ...
  test("mesos does not acquire gpus if not specified") {
    ...
    val gpus = executorProvider.getResource(taskInfos.head.getResourcesList, "gpus")
    assert(gpus == 0.0)
  }
  ...
  test("mesos declines offers that violate attribute constraints") {
    ...
    offerResources(List(Resources(executorProvider.executorMemory(sc), 4)))
  ...
  test("mesos doesn't register twice with the same shuffle service") {
    ...
    val (mem, cpu) = (executorProvider.executorMemory(sc), 4)

    val offer1 = createOffer("o1", "s1", mem, cpu)
    executorProvider.resourceOffers(driver, List(offer1).asJava)
    verifyTaskLaunched(driver, "o1")

    val offer2 = createOffer("o2", "s1", mem, cpu)
    executorProvider.resourceOffers(driver, List(offer2).asJava)
    verifyTaskLaunched(driver, "o2")

    val status1 = createTaskStatus("0", "s1", TaskState.TASK_RUNNING)
    executorProvider.statusUpdate(driver, status1)

    val status2 = createTaskStatus("1", "s1", TaskState.TASK_RUNNING)
    executorProvider.statusUpdate(driver, status2)
  ...
  test("mesos kills an executor when told") {
    ...
    executorProvider.doKillExecutors(List("0"))
  ...
  test("Do not call removeExecutor() after backend is stopped") {
    ...
    // launches a task on a valid offer
    val offers = List(Resources(executorProvider.executorMemory(sc), 1))
    offerResources(offers)
    verifyTaskLaunched(driver, "o1")
  ...
  private def offerResources(offers: List[Resources], startId: Int = 1): Unit = {
    ...
    executorProvider.resourceOffers(driver, mesosOffers.asJava)
  }
  ...
  private def createSchedulerBackend(
    ...
    val configuredBackend = new CoarseGrainedSchedulerBackend(
      ...
      override def createDriverEndpointRef(ignored: ThreadSafeRpcEndpoint): RpcEndpointRef
        = endpoint
    }
    configuredBackend.start()
    configuredBackend
  }
