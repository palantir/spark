...
import org.apache.spark.{SPARK_VERSION, SparkContext, SparkException}
...
private[spark] class KubernetesExecutorProvider(
    val sc: SparkContext)
  extends ClusterManagerExecutorProvider with Logging {

  import KubernetesExecutorProvider._
  ...
  private val executorDockerImage = conf
    .get("spark.kubernetes.executor.docker.image", s"spark-executor:$SPARK_VERSION")
  ...
  override def start(): Unit = {
    ...
    if (!Utils.isDynamicAllocationEnabled(sc.conf)) {
      doRequestTotalExecutors(initialExecutors, 0, Map.empty[String, Int])
    }
  }
  ...
  override def doRequestTotalExecutors(requestedTotal: Int,
      ...
      hostToLocalTaskCount: Map[String, Int]): Future[Boolean] = Future[Boolean] {
...
private object KubernetesExecutorProvider {
